...
export const getWindowState = (
  ...
  try {
    ...
    logger.error(`window state 파일 읽기에 실패. [${e.message}]`);
    return defaultState;
  }
...
const setWindowState = <k extends keyof WindowState>(
  ...
  try {
    ...
    // silent fail
    logger.error(`윈도우 상태 쓰기를 실패함. [${e.message}]`);
    return;
  }
};
...
const saveSize = (targetWindow: BrowserWindow) => {
  ...
  logger.debug(`윈도우 크기 기록함 [width : ${width} , height : ${height}]`);
};
...
const savePosition = (targetWindow: BrowserWindow) => {
  ...
  logger.debug(`윈도우 위치 기록함 [x : ${x} , y : ${y}]`);
};
